...
public abstract class Board {
    ...
    public boolean addZombie(Zombie zombie, Point position) {
        if (position.x >= columns || position.y >= rows || zombies.containsKey(position)) {
            return false;
        }
    ...
    public void processTurn() {
        for (Plant plant : plants.values()) {
            if (plant.getType().equals("Offensive")) {
                ((OffensivePlant) plant).attack(this);
            }
        }
    }
    ...
    public void useTool(Tool tool, Point position) {
